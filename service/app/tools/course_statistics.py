...
from app.tools.pdf_reader import get_student_results
...
def get_course_statistics(pdf_file):
    ...
    if from_scrape:
        ...
        courses_data.save()
    student_results = get_student_results(pdf_file, course_database)
...
def main(argv):
    ...
    course_database = CourseDatabase(from_pkl=True, from_scrape=False,
        from_year=FROM_SCHOOL_YEAR, to_year=TO_SCHOOL_YEAR, program=PROGRAM)
    #courses_data.save()
    student_results = get_student_results(os.path.abspath(pdf_file), course_database)
